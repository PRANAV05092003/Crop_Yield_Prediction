...
This repo includes a **production-ready web app** that serves the trained yield model via an API and a Next.js frontend.

- **Backend**: FastAPI, loads the Phase3 RandomForest yield model, exposes `GET /` (health) and `POST /predict` (prediction + confidence).
- **Frontend**: Next.js 14 (App Router), TypeScript, Tailwind, ShadCN-style UI, Framer Motion. Landing page, prediction form, dark/light mode, toasts, loading and result display.
- **Deploy**: Backend → Render or Railway; Frontend → Vercel. See **[DEPLOYMENT.md](./DEPLOYMENT.md)** for step-by-step instructions and environment variables.

### Quick start (local)

**Full step-by-step guide:** see **[LOCAL_SETUP.md](./LOCAL_SETUP.md)** (recommended for first-time setup).

**Short version** (from project root):

```bash
# 1. Backend — install deps & train model (once)
py -m pip install -r backend/requirements.txt
py backend/train_model.py

# 2. Backend — start API (keep this terminal open)
cd backend
...
```bash
# 3. Frontend — install, set API URL, run
cd frontend
npm install
...
Then open **http://localhost:3000** and use **Predict** to submit weather + NDVI data.

### API overview

| Endpoint    | Method | Description |
|------------|--------|-------------|
| `/`        | GET    | Health check; returns `{ "status": "ok", "model": "loaded" }` when model is ready. |
| `/predict` | POST   | JSON body with weather and NDVI fields (see `/docs`). Returns `{ "prediction": <yield>, "confidence": <0-1> }`. |
| `/docs`    | GET    | Swagger UI. |

Environment variables: **Backend**: `PORT`, `CORS_ORIGINS`. **Frontend**: `NEXT_PUBLIC_API_URL`. See `.env.example` in `backend/` and `frontend/`, and [DEPLOYMENT.md](./DEPLOYMENT.md).

---

## Research and model (original repo)
